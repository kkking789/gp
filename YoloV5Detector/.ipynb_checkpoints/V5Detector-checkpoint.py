...
from utils.general import (LOGGER, Profile, cv2,non_max_suppression)
    ...
    def draw_box(self, im, pred_boxes):
        if not pred_boxes:
            ...
            for pred_box in pred_boxes:
                Annotator.box_label(pred_box, im, label=pred_box[5]+" "+str(pred_box[6]), color=self.colors_random[int(pred_box[4])], line_thickness=3)
        return im
